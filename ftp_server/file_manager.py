...
class FileManager:
    ...
    def dir_size(self, path_):
        size = 0
        for path, dirs, files in os.walk(path_):
            for dir_ in dirs:
                size += self.dir_size(os.path.join(path, dir_))
            for file in files:
    ...
    def mkdir(self, path):
        if self.checkPath(path):
            os.mkdir(path)
            if self.dir_size(self.root) > MAX_DIRECTORY_SIZE:
                self.rm(path)
    ...
    def free(self):
        return f'Всего места: {MAX_DIRECTORY_SIZE}\nСвободно: {MAX_DIRECTORY_SIZE - self.dir_size(self.root)}\n'
...
def handle(sock, conn, login):
    ...
    while True:
        try:
            ...
            if request == 'exit':
                conn.close()
            response = fileManager.processing(request)
            send(conn, response)
